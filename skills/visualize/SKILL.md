---
name: visualize
description: "Add a correct, minimal visual to a lesson — a diagram or geometric picture — that renders inline in the linked Markdown lesson. Use when an idea is genuinely clearer as a picture: a dependency graph, system/flow, sequence, state machine, tree, comparison, or a spatial/geometric thing (coordinate geometry, number line, vectors, a plot, a physical layout). Outsources authoring+rendering to a maker agent that verifies the image by looking at it, then you embed the returned file."
---

# Visualize

A picture earns its place only when it shows something words can't — shape, structure, direction, relationship, geometry. This skill produces ONE such picture, guarantees it is **correct** (the maker renders it and looks at it before returning), and drops it into the lesson so it renders inline in the linked Markdown file.
...
Two makers, described in `agents/` and exposed through the provider-native
agent definitions:
...
Dispatch the maker using the provider's native agent/delegation mechanism:

```
agent: mermaid-maker
task: <your minimal, concrete brief>
```
```
agent: svg-maker
task: <your minimal, concrete brief>
...
The session logging protocol in `AGENTS.md` keeps the linked Markdown lesson
append-only. Store the published image in the project's `viz` folder so the
embed resolves from the lesson. Width `|500` is a good default; use larger for
dense diagrams. Introduce the visual in a sentence, then let it carry the idea
— don't narrate every element back in prose.
...
> Rendering is provider-dependent. Use the available Mermaid/SVG renderer and
> inspect the output before publishing; never claim a visual was verified if
> the current provider cannot render or inspect it.
